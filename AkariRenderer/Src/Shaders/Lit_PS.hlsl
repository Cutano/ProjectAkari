...
float3 ImageBasedPBRLighting(float3 baseColor, float3 V, float3 N, float3 F0, float roughness, float metallic, float ao)
{
	...
	float2 dfg = IBLTexture.SampleLevel(LinearClampSampler, float2(saturate(dot(N, V)), roughness), 0).rg;
	float3 specularDFG = lerp(dfg.xxx, dfg.yyy, F0);
	float3 specular = prefilteredColor * specularDFG;
	float3 energyCompensation = 1.0 + F0 * (1.0 / dfg.y - 1.0);
	
	float3 ambient = (kD * diffuse + specular * energyCompensation) * ao;
...
SurfaceShadingData GetSurfaceData(VertexShaderOutput psInput)
{
	...
	if (MaterialCB.HasNormalTexture)
	{
		...
		o.Bitangent = normalize(bitangent);
	}
	else
	{
		o.Normal = normalize(psInput.NormalWS);
		o.Tangent = normalize(psInput.TangentWS);
		o.Bitangent = normalize(psInput.BitangentWS);
	}
