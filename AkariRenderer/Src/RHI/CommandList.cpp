...
ComPtr<ID3D12Resource> CommandList::CopyBuffer( size_t bufferSize, const void* bufferData, D3D12_RESOURCE_FLAGS flags )
{
    ...
    else
    {
        ...
        if ( bufferData != nullptr )
        {
            ...
            const auto uploadResourceDesc = CD3DX12_RESOURCE_DESC::Buffer( bufferSize );
            ThrowIfFailed( d3d12Device->CreateCommittedResource(
...
void CommandList::PrefilterCubeMap(const std::shared_ptr<Texture>& texture)
{
    assert(texture);
    ...
    rootParameters[1].InitAsDescriptorTable( 1, &outMip );
    ...
    pipelineStateStream.CS             = { g_PrefilterEnvMap_CS, sizeof( g_PrefilterEnvMap_CS ) };
    ...
    auto uavTexture = m_Device.CreateTexture( uavResource );
    uavTexture->SetName(L"UAV Texture");
    ...
    for ( uint32_t mip = 0; mip < 10; ++mip )
    {
        ...
        auto uav = m_Device.CreateUnorderedAccessView( uavTexture, nullptr, &uavDesc );
                                          ...
                                          uint32_t numSubresources, D3D12_SUBRESOURCE_DATA* subresourceData )
{
    ...
    if ( destinationResource )
    {
        ...
        const auto uploadResourceDesc = CD3DX12_RESOURCE_DESC::Buffer( requiredSize );
        ThrowIfFailed( d3d12Device->CreateCommittedResource(
...
void CommandList::SetIndexBuffer( const std::shared_ptr<IndexBuffer>& indexBuffer )
{
    if ( indexBuffer )
    {
        ...
        const auto bufferView = indexBuffer->GetIndexBufferView();
